...
private fun Project.configureComponentPublishing(
    ...
    configure<PublishingExtension> {
        repositories {
            it.maven { repo ->
                repo.apply {
                    setUrl(findProperty("maven.tencent.url").toString())
                    credentials {
                        it.username = findProperty("maven.tencent.username")?.toString()
                        it.password = findProperty("maven.tencent.password")?.toString()

                        if (it.username.isNullOrBlank() || it.password.isNullOrBlank()) {
                            logger.warn("""
                                |Maven credentials are not properly configured.
                                |Get them here: ${findProperty("maven.tencent.manager.url")}.
                            """.trimMargin())
                        }
                    }
                }

...
fun enableArtifactRedirectingPublishing(project: Project) {
    ...
    ext.targets.all { target ->
        println("GavinTag: 实现 Publish 依赖 Task: project=${project.name} target=${target.name} oelTargetNames=$oelTargetNames isAndroidTarget=${target is KotlinAndroidTarget} newRootComponent=$newRootComponent")

...
class CupertinoOverscrollEffect(
    ...
    // Detect and immediately consume the drag gesture without awaiting slop in overscroll area.
    private fun Modifier.draggableOverscroll() = this.then(
        Draggable2DElement(
            enabled = true,
            reverseDirection = false,
            interactionSource = null,
            onDragStarted = {},
            // Returns true if the pointer is in the overscroll area.
            canDrag = canDrag@{
                // Returns false if there is no overscroll area.
                if (overscrollOffset == Offset.Zero) return@canDrag false
                val pointerPosition = it.position
                val screenRect = scrollSize.toRect()
                val scrollRect = screenRect.translate(visibleOverscrollOffset.toOffset())
                val visibleRect = screenRect.intersect(scrollRect)
                val isInOverscrollArea =
                    pointerPosition in screenRect && pointerPosition !in visibleRect
                isInOverscrollArea
            },
            startDragImmediately = { isInProgress },
            state = Draggable2DState { /* Do nothing. Just consume the drag. */ },
            onDragStopped = { /* Do nothing. Just consume the drag. */ },
        )
    )

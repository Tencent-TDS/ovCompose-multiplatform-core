...
internal class MouseWheelScrollingLogic(
    ...
    private val onScrollStopped: (velocity: Velocity) -> Unit,
    ...
    fun onPointerEvent(pointerEvent: PointerEvent, pass: PointerEventPass, bounds: IntSize) {
        ...
        if (pass == PointerEventPass.Main && pointerEvent.type == PointerEventType.ScrollEnd) {
            val velocity = velocityTracker.calculateVelocity()
            onScrollStopped(
                Velocity(
                    x = if (abs(velocity.x) > VelocitySlop) velocity.x else 0f,
                    y = if (abs(velocity.y) > VelocitySlop) velocity.y else 0f
                )
            )
    ...
    private fun ScrollingLogic.canConsumeDelta(scrollDelta: Offset): Boolean {
        ...
        return delta != 0f // It means that it's for another axis and cannot be consumed
    }
    ...
    private fun NestedScrollScope.dispatchMouseWheelScroll(delta: Float) =
        with(scrollingLogic) {
            ...
            val consumed = scrollBy(offset, NestedScrollSource.Wheel)
...
private const val ScrollProgressTimeout = 0L // ms
private const val VelocitySlop = 1000 // velocity slop

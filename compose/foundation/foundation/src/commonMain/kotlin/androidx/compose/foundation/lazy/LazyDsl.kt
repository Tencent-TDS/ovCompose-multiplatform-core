...
// region Tencent Code
// add beyondBoundsItemCount param to LazyColumn
/**
 * The vertically scrolling list that only composes and lays out the currently visible items.
 * The [content] block defines a DSL which allows you to emit items of different types. For
 * example you can use [LazyListScope.item] to add a single item and [LazyListScope.items] to add
 ...
 * is allowed. You can still scroll programmatically using the state even when it is disabled
 * @param overscrollEffect the overscroll effect applied to the LazyColumn.
 * @param beyondBoundsItemCount compose and layout before and after the list of visible items.
 *  Be aware that using a large value for [beyondBoundsItemCount] will cause a lot of items to be
 *  compose, measured and placed which will defeat the purpose of using lazy loading.
 *  This should be used as an optimization to pre-load a couple of items before and after the visible ones.
 * @param content a block which describes the content. Inside this block you can use methods like
 * [LazyListScope.item] to add a single item or [LazyListScope.items] to add a list of items.
 */
@ExperimentalFoundationApi
...
fun LazyColumn(
    ...
    require(beyondBoundsItemCount >= 0) {
        "beyondBoundsItemCount should be greater than or equal to 0, you selected $beyondBoundsItemCount"
    }
    LazyList(
        modifier = modifier,
        state = state,

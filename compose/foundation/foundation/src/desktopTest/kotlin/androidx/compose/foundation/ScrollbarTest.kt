...
class ScrollbarTest {
    ...
    fun `thumb size on scrollbar smaller than viewport`() {
        ...
        // should do nothing.
        for (offset in 1..50){
            rule.onNodeWithTag("scrollbar").pressWaitForIdleRelease(Offset(0f, offset.toFloat()))
            assertEquals(0, scrollState.value)
        }

        // Clicking one pixel below the thumb should scroll the content by one viewport
        rule.onNodeWithTag("scrollbar").pressWaitForIdleRelease(Offset(0f, 51f))
...
//    }
    ...
    fun `press on the end of scrollbar outside slider`(scrollbarProvider: ScrollbarProvider) {
        ...
        // Pressing at the bottom should cause the scrollbar to scroll to the bottom
        rule.onNodeWithTag("scrollbar").performMouseInput {
    ...
    fun `basic lazy grid scrollbar test`() {
        ...
        rule.onNodeWithTag("scrollbar").pressWaitForIdleRelease(Offset(0f, 0f))
        rule.onNodeWithTag("box0").assertTopPositionInRootIsEqualTo(0.dp)
    }
    ...
    fun `basic text field with vertical scrolling test`() {
        ...
        scrollState.assertChangeInOffset(0f){
            rule.onNodeWithTag("scrollbar").pressWaitForIdleRelease(Offset(0f, 5f))
        }

        // But clicking on the 4th pixel should scroll to top
        scrollState.assertChangeInOffset(-scrollState.offset){
            rule.onNodeWithTag("scrollbar").pressWaitForIdleRelease(Offset(0f, 4f))
        }
        ...
        // do nothing
        scrollState.assertChangeInOffset(0f){
            rule.onNodeWithTag("scrollbar").pressWaitForIdleRelease(Offset(0f, 50f))
        }

        // But clicking on the 49th pixel should scroll to the very top
        scrollState.assertChangeInOffset(-scrollState.offset){
            rule.onNodeWithTag("scrollbar").pressWaitForIdleRelease(Offset(0f, 49f))
        }
    }
    ...
    fun `basic text field with horizontal scrolling test`() {
        ...
        scrollState.assertChangeInOffset(0f){
            rule.onNodeWithTag("scrollbar").pressWaitForIdleRelease(Offset(99f, 0f))
        }
    ...
    /**
     * A "click" for pressing the scrollbar outside its thumb.
     *
     * Note: `MouseInjectionScope` doesn't work because `Modifier.scrollOnPressOutsideThumb`
     * requires a recomposition to happen between the press and release.
     */
    private fun SemanticsNodeInteraction.pressWaitForIdleRelease(position: Offset){
        performMouseInput {
            moveTo(position)
            press()
        }

...
import androidx.compose.ui.window.LocalChoreographer
...
internal actual fun LazyLayoutPreFetcherProxy(
    ...
    val getTargetDrawingTimeNs: () -> Long = remember(choreographer) { choreographer::getTargetDrawingTimeNs }
    val getLastFrameDrawingTimeNs: () -> Long = remember(choreographer) { choreographer::getLastFrameTimeNs }
    val getDurationTimeNs: () -> Long = remember(choreographer) { choreographer::getDurationTimeNs }

    remember(prefetchState, subcomposeLayoutState, itemContentFactory, getTargetDrawingTimeNs) {
        LazyLayoutPrefetcher(
            prefetchState,
            subcomposeLayoutState,
            itemContentFactory,
            getTargetDrawingTimeNs,
            getLastFrameDrawingTimeNs,
            getDurationTimeNs
        )
    }
}
...
internal class LazyLayoutPrefetcher(
    ...
    private val getTargetDrawingTimeNs: () -> Long,
    private val getLastFrameDrawingTimeNs: () -> Long,
    private val getDurationTimeNs: () -> Long
) : RememberObserver,
    ...
    private val averageCompositionTimeNanosByContentType = mutableObjectLongMapOf<Any>()
    private val averageMeasureTimeNanosByContentType = mutableObjectLongMapOf<Any>()
    ...
    override fun run() {
        ...
        val nextFrameNs =  getTargetDrawingTimeNs()
        var oneOverTimeTaskAllowed = currentTimeNs < nextFrameNs
        ...
        while (prefetchRequests.isNotEmpty() && !scheduleForNextFrame) {
            ...
            if (request.canceled || request.index !in 0 until itemProvider.itemCount) {
                ...
                val averagePrecomposeTimeNs = getAveragePrecomposeTimeNs(contentType)
                if (enoughTimeLeft(
                        beforeTimeNs,
                        nextFrameNs,
                    ...
                    PerformanceTrace.traceSync(TraceScene.PreCompose.value) {
                        request.precomposeHandle = subcomposeLayoutState.precompose(key, content)
                    }
                    setAveragePrecomposeTimeNs(
                        contentType,
                        calculateAverageTime(nanoTime() - beforeTimeNs, averagePrecomposeTimeNs)
                    )
                } else {
                ...
                val beforeTimeNs = nanoTime()
                val contentType = itemProvider.getContentType(request.index)
                val averagePremeasureTimeNs = getAveragePremeasureTimeNs(contentType)
                if (enoughTimeLeft(beforeTimeNs, nextFrameNs, averagePremeasureTimeNs) ||
                    ...
                    repeat(handle.placeablesCount) { placeableIndex ->
                        PerformanceTrace.traceSync(TraceScene.PreMeasure.value) {
                            handle.premeasure(
                                placeableIndex,
                                request.constraints
                            )
    ...
    private fun getAveragePrecomposeTimeNs(contentType: Any?): Long {
        contentType?.let {
            return averageCompositionTimeNanosByContentType.getOrDefault(it, 0L)
        }
        return 0L
    }

    private fun setAveragePrecomposeTimeNs(contentType: Any?, timeNs: Long) {
        contentType?.let { averageCompositionTimeNanosByContentType[it] = timeNs }
    }

    private fun getAveragePremeasureTimeNs(contentType: Any?): Long {
        contentType?.let {
            return averageMeasureTimeNanosByContentType.getOrDefault(it, 0L)
        }
        return 0L
    }

    private fun setAveragePremeasureTimeNs(contentType: Any?, timeNs: Long): Long {
        contentType?.let {
            return averageMeasureTimeNanosByContentType.getOrDefault(it, timeNs)
        }
        return 0L
    }

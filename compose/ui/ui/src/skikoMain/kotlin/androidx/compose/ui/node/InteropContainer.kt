...
internal fun <T> InteropContainer<T>.countInteropComponentsBefore(nativeView: T): Int {
    ...
    rootModifier?.takeIf { it.isAttached }?.traverseDescendants {
 ...
 */
@Composable
fun <T> InteropContainer<T>.TrackInteropContainer(content: @Composable () -> Unit) {
    OverlayLayout(
 ...
 */
data class TrackInteropModifierElement<T>(
 ...
 * @see TraversableNode
 */
class TrackInteropModifierNode<T>(
    var nativeView: T?

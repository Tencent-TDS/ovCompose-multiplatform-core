...
internal fun SemanticsNode.sortByGeometryGroupings(
    ...
    for (entryIndex in 0..parentListToSort.lastIndex) {
        ...
        if (entryIndex == 0 || !placedEntryRowOverlaps(rowGroupings, currEntry, isIOS)) {
     ...
     *     }
     */
    val iosFriendlyComparator = Comparator<SemanticsNode> { nodeA, nodeB ->

        val boundsA = nodeA.boundsInWindow
        val boundsB = nodeB.boundsInWindow
        val overlaps = boundsA.overlaps(boundsB)

        if (overlaps) {
            val zIndexA = zIndexMap.getValue(nodeA.id)
            val zIndexB = zIndexMap.getValue(nodeB.id)
            return@Comparator zIndexB.compareTo(zIndexA)
        }
        return@Comparator comparator.compare(nodeA, nodeB)
    }

    val finalComparator = if (isIOS) {
        iosFriendlyComparator
    } else {
        comparator
    }
    ...
    rowGroupings.fastForEach { row ->
        // Sort each individual row's parent nodes
        row.second.sortWith(finalComparator)
        returnList.addAll(row.second)
    }
    // endregion

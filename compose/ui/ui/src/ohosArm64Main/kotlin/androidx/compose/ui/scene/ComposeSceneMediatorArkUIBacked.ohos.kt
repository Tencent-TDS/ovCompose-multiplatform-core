...
import androidx.compose.ui.arkui.ArkUITouchEvent
import androidx.compose.ui.arkui.AxisEvent
import androidx.compose.ui.arkui.InternalArkUIViewControllerArkUIBacked
...
import androidx.compose.ui.interop.arkc.destroy
import androidx.compose.ui.interop.arkc.position
...
internal class ComposeSceneMediatorArkUIBacked(
    private val controller: InternalArkUIViewControllerArkUIBacked,
    ...
    internal var nativeEvent: ArkUIInputEvent = null
    ...
    fun onDraw(canvas: Canvas, timeStamp: Long) {
        scene.render(canvas.asComposeCanvas(), timeStamp)
    ...
    fun sendPointerEvent(env: napi_env, event: napi_value): Boolean {
        ...
        nativeEvent.position *= scene.density.density
        trace("sendPointerEvent") {
            scene.sendPointerEvent(
                eventType = eventType,
                pointers = pointers,
                timeMillis = event.timestamp,
                nativeEvent = ArkUITouchEvent(nativeEvent, TouchEvent(event))
            )
        }
        nativeEvent.destroy()
        nativeEvent = null

...
import androidx.compose.ui.arkui.extra.set
...
import androidx.compose.ui.arkui.performance.resumeGC
import androidx.compose.ui.arkui.performance.suspendGC
...
import platform.arkui.ArkUI_NodeContentHandleVar
...
internal sealed interface InternalArkUIViewController : ArkUIViewController, MessengerOwner, IVsyncProxy {
    ...
    var id: String
    var env: napi_env?
    var internalContext: ContextImpl?
    var internalUiContext: UIContextImpl?
    var backRootView: ArkUIRootView?
    var foreRootView: ArkUIRootView?
    var touchableRootView: ArkUIRootView?
    val backNativeInteropContainer: ArkUINativeInteropContainer?
    val foreNativeInteropContainer: ArkUINativeInteropContainer?
    val touchableNativeInteropContainer: ArkUINativeInteropContainer?

    override val messenger: MessengerImpl

    /** 当前 Surface 是否激活；激活时能渲染画面，非激活时不能渲染画面 */
    val isSurfaceActive: Boolean
    ...
    fun onFocusEvent() {}
    fun onKeyEvent() {}
    fun onKeyEvent(event: napi_value): Boolean = false
    ...
    fun onDraw(timestamp: Long, targetTimestamp: Long, renderStrategy: RenderStrategy) {}
    fun onFinalize() {}
    fun bindContext(context: ArkUIInteropContext) {}
}
...
internal abstract class BasicArkUIViewController(
    ...
    override val backNativeInteropContainer = nativeInteropContainer()
    override val foreNativeInteropContainer = nativeInteropContainer()
    override val touchableNativeInteropContainer = nativeInteropContainer()
    ...
    override fun onFrame(timestamp: Long, targetTimestamp: Long) {
        ...
        if (isSurfaceActive) {
            if (active || forceInvalid) {
                forceInvalid = false
                onSystemVsync(timestamp)
                if (syncInvalid) {
        ...
        configuration.delegate.onSurfaceFrame(timestamp, targetTimestamp)
    }

    override fun bindContext(context: ArkUIInteropContext) {
        renderStrategy.config.interopContext = context
    }

    private fun onSystemVsync(timestamp: Long) {
        OhosTrace.traceSync(TRACE_SYSTEM_VSYNC) {
            renderStrategy.onSystemVsync(timestamp)
        }
    }


    private fun onComposeVsync(timestamp: Long) {
        OhosTrace.traceSync(TRACE_COMPOSE_VSYNC) {
            ChoreographerManager.onVsync(timestamp)
        }
    }

    private fun onFrameInner(timestamp: Long, targetTimestamp: Long) {
        if (!invalid || !isSurfaceActive || !isComponentActive) {
            return
        }
        invalid = false

        suspendGC()
	    renderStrategy.prepareRender()

        drawingTime = timestamp
        onDraw(timestamp, targetTimestamp, renderStrategy)

	    renderStrategy.finishRender()
        resumeGC()
    }
    ...
    override fun onSurfaceCreated(component: OHNativeXComponent, width: Int, height: Int) {
        ...
        invalid = true
        forceInvalid = true
        configuration.delegate.onSurfaceCreated(width, height)
    }

    override fun onSurfaceChanged(width: Int, height: Int) {
        super.onSurfaceChanged(width, height)
        configuration.delegate.onSurfaceChanged(width, height)
    }
    ...
    companion object {
        private const val TRACE_SYSTEM_VSYNC = "OnSystemVsync"
        private const val TRACE_COMPOSE_VSYNC = "OnComposeVsync"
    }
}
...
fun _ArkUIViewController_setXComponentRender(controllerRef: COpaquePointer, render: COpaquePointer) {
    controllerRef.getController()?.renderStrategy?.render = render
}

@InternalComposeApi
fun _ArkUIViewController_getXComponentRender(controllerRef: COpaquePointer): COpaquePointer? =
    controllerRef.getController()?.renderStrategy?.render
...
@InternalComposeApi
fun _ArkUIViewController_setNativeInteropContainer(
    controllerRef: COpaquePointer,
    backContainer: napi_value,
    foreContainer: napi_value,
    touchContainer: napi_value
) {
    fun setupInteropContainer(
        interopContent: napi_value,
        interopContainer: ArkUINativeInteropContainer?
    ) {
        if (interopContainer != null) {
            val handle = interopContainer.handle
            val nodeContent = memScoped {
                val content = alloc<ArkUI_NodeContentHandleVar>()
                OH_ArkUI_GetNodeContentFromNapiValue(JsEnv.env(), interopContent, content.ptr)
                content.value
            }
            OH_ArkUI_NodeContent_AddNode(nodeContent, handle)
        }
    }

    controllerRef.getController()?.run {
        setupInteropContainer(backContainer, backNativeInteropContainer)
        setupInteropContainer(foreContainer, foreNativeInteropContainer)
        setupInteropContainer(touchContainer, touchableNativeInteropContainer)
    }
}
...
// ArkUI 的 Compose 组件被移除时的回调
@InternalComposeApi
fun _ArkUIViewController_aboutToDisappear(controllerRef: COpaquePointer) {
    val controller = controllerRef.getController()
    // ArkUI 的 Compose 节点被拔除了，我们认为 Compose 节点不会再复用了，所以取消 DisposableRef 的引用；注意不是取消 StableRef 的引用，因为 StableRef 可能还会被读取
...
fun _ArkUIViewController_onAxisEvent(controllerRef: COpaquePointer, event: napi_value) {
    controllerRef.getController()?.onAxisEvent(event)
}

@InternalComposeApi
fun _ArkUIViewController_onKeyEvent(controllerRef: COpaquePointer, event: napi_value): Boolean {
    return controllerRef.getController()?.onKeyEvent(event) ?: false
}

@InternalComposeApi
fun _ArkUIViewController_onMouseEvent(controllerRef: COpaquePointer, event: napi_value) {
    controllerRef.getController()?.onMouseEvent(event)
}

@InternalComposeApi
fun _ArkUIViewController_onPinchEvent(controllerRef: COpaquePointer, event: napi_value) {
    controllerRef.getController()?.onPinchEvent(event)
}
...
@InternalComposeApi
fun _ArkUIViewController_updateDensity(controllerRef: COpaquePointer, density: Float) {
    controllerRef.getController()?.updateDensity(density)
}
...
// ArkUI 的 Compose 组件中的 ArkUIViewController JS 对象被 JS 运行时回收后的回调
@InternalComposeApi
fun _ArkUIViewController_onFinalize(controllerRef: COpaquePointer) {
    controllerRef.getController()?.onFinalize()
    // ArkUIViewController JS 对象已经不再使用了，也取消 Kotlin 的 StableRef 对 DisposableRef<BasicArkUIViewController> 的引用
    controllerRef.asStableRef<DisposableRef<BasicArkUIViewController>>().dispose()
}

@OptIn(ExperimentalComposeApi::class)
@InternalComposeApi
fun _ArkUIViewController_setExtraParam(controllerRef: COpaquePointer, key: String, value: String) {
    controllerRef.getController()?.extras?.set(key, value)
}
...
private inline fun COpaquePointer.getController(): InternalArkUIViewController? =
    asStableRef<DisposableRef<InternalArkUIViewController>>().get().get()
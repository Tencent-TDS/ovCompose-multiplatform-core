...
object JsEnv {
    ...
    fun arrayOf(vararg values: napi_value?): napi_value? {
        return memScoped {
            val result = alloc<napi_valueVar>()
            checkStatus(napi_create_array_with_length(env(), values.size.toULong(), result.ptr))
            values.forEachIndexed { index, value ->
                checkStatus(napi_set_element(env(), result.value, index.toUInt(), value))
            }
            result.value
        }
    }

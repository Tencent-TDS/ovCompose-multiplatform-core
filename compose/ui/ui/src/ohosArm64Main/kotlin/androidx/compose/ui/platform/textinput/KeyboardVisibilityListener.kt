...
internal class KeyboardVisibilityListenerImpl(
    ...
    private val getSceneViewHeight: () -> Int,
    ...
    override fun keyboardWillShow(keyboardHeight: Float) {
        ...
        if (focusedRect != null) {
            keyboardAvoidFocusOffsetState.value = with(density()) {
                calcFocusedLiftingY(getSceneViewHeight, focusedRect, keyboardHeight).toDp()
            }
        }
    }
    ...
    private fun calcFocusedLiftingY(
        getSceneViewHeight: () -> Int,
        focusedRect: Rect,
        keyboardHeight: Float
    ): Int {
        val viewHeight = getSceneViewHeight()

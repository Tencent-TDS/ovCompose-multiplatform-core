...
internal abstract class Redrawer(protected val callbacks: RedrawerCallbacks) {
                ...
                if (ComposeTabService.sendTouchesAsynchronously) {
                    ...
                    dispatch_async(dispatch_get_main_queue()) {
                        if (!isDisposed) {
                            ...
                            draw(waitUntilCompletion = false, targetTimestamp, duration, lastFrameTimestamp)
                        }
                    }
                } else {
                    draw(waitUntilCompletion = false, targetTimestamp, duration, lastFrameTimestamp)
    ...
    private val currentTargetTimestamp: NSTimeInterval
        get() = caDisplayLink?.targetTimestamp ?: 0.0

    private val lastFrameTimestamp: NSTimeInterval
        get() = caDisplayLink?.timestamp ?: 0.0

    private val duration: NSTimeInterval
        get() = caDisplayLink?.duration ?: 0.0
    ...
    protected abstract fun draw(waitUntilCompletion: Boolean, targetTimestamp: NSTimeInterval, duration: NSTimeInterval, lastFrameTimestamp: NSTimeInterval)
    ...
    fun drawSynchronously() {
        ...
        // why use CACurrentMediaTime() instead of targetTimestamp?
        draw(waitUntilCompletion = true, CACurrentMediaTime(), duration, lastFrameTimestamp)
    }
}
...
internal class ApplicationStateListener(
    ...
    init {
        ...
        dispatch_async(dispatch_get_main_queue()) {
            ...
            val isActive = currentState != UIApplicationState.UIApplicationStateBackground
            callback(isActive)
        }
        // endregion
    }
...
internal interface RedrawerCallbacks {
     ...
     */
    fun render(canvas: Canvas, targetTimestamp: NSTimeInterval, duration: NSTimeInterval, lastFrameTimestamp: NSTimeInterval)

...
import androidx.compose.ui.semantics.NonFocusableAccessibilityKey
...
fun <T : UIView> UIKitView(
    ...
    onResize: (view: T, rect: CValue<CGRect>) -> Unit = DefaultViewResize,
    interactive: Boolean = true,
    highestPriorityResponseTouch: Boolean = false
) {
    // region Tencent Code
    val renderBackend = LocalRenderBackend.current
    when (renderBackend) {
        RenderBackend.Skia -> UIKitViewForSkiaBackend(factory, modifier, update, background, onRelease, onResize, interactive)
        RenderBackend.UIView -> UIKitViewForUIViewBackend(factory, modifier, update, background, onRelease, onResize, interactive, highestPriorityResponseTouch)
    }
    // endregion
}
...
fun <T : UIView> UIKitView2(
    ...
    when (renderBackend) {
        RenderBackend.Skia -> UIKitViewForSkiaBackend(factory, modifier, update, background, onRelease, onResize, interactive)
        RenderBackend.UIView -> UIKitViewForUIViewBackend(factory, modifier, update, background, onRelease, onResize, interactive, false,true)
    }
}
    ...
    Place(
        ...
        modifier = Modifier.then(modifier)
            ...
            .uiViewLayer { view ->
                ...
                if (interactive) {
                    it.pointerInteropFilter(wrappingView = embeddedInteropComponent.wrappingView, highestPriorityResponseTouch)
                } else {
                    it
                }
            }
...
private class EmbeddedInteropForUIViewBackend<T : UIView>(
    ...
    fun removeFromHierarchy() {
        wrappingView.setOnSizeChange(null)
...
internal val NativeAccessibilityViewSemanticsKey = NonFocusableAccessibilityKey<TMMInteropWrapView>(
 ...
 * Also adds inspectorInfo with interopId for LayoutInspector support.
 *
...
fun Modifier.nativeAccessibility(isEnabled: Boolean, interopWrappingView: TMMInteropWrapView) =
    this.inspectable(
        inspectorInfo = debugInspectorInfo {
            name = "nativeAccessibility"
            properties["interopId"] = interopWrappingView.interopId
        }
    ) {
        if (isEnabled) {
            semantics { nativeAccessibilityView = interopWrappingView }
        } else {
            this
        }
    }
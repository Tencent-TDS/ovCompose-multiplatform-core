...
import androidx.compose.ui.uikit.utils.TMMCanvasViewProxyV3
...
/* 用于 translate 中判断浮点数是否过小 */
private inline fun Float.safeEqualToZero() : Boolean {
    return abs(this) < 0.0000001
}
...
@HiddenFromObjC
internal class AdaptiveCanvas(private val experimentalConfig: ExperimentalConfig?) : IOSNativeCanvas {

    override val canvasType: CanvasType get() = CanvasType.Native

    val viewProxy: ITMMCanvasViewProxyProtocol = createViewProxy(experimentalConfig)

    private val nativePaint = viewProxy.paint()

    private val nativeViewPtr = viewProxy.nativePtr()

    private inline fun createViewProxy(experimentalConfig: ExperimentalConfig?): ITMMCanvasViewProxyProtocol {
        val experimentalConfigPtr: Long = if (experimentalConfig != null) {
            (experimentalConfig as ExperimentalConfigImpl).nativeExperimentalConfigPtr
        } else {
            0L
        }
        return TMMCanvasViewProxyV3.canvasViewProxyV3WithExperimentalConfigPtr(experimentalConfigPtr)
    }

    override fun onBeginComponentDraw(semanticsId: Int) {
        // 通知 OC 层标记后续创建的 CALayer
        viewProxy.beginComponentDraw(semanticsId)
    }

    override fun onEndComponentDraw(semanticsId: Int) {
        // 通知 OC 层当前组件绘制结束
        viewProxy.endComponentDraw(semanticsId)
    }
    ...
    override fun experimentalConfig(): ExperimentalConfig? {
        return experimentalConfig
    }

    override fun imageFromCanvasPtr(
        nativeCGBitmapCanvasPtr: COpaquePointer,
        colorArgb: Int
    ): Long {
        return viewProxy.imageFromCanvasPtr(
            nativeCGBitmapCanvasPtr, colorArgb
        )
    }
    ...
    override fun translate(dx: Float, dy: Float) {
        // 这里是 dx 和 dy 是增量偏移，如果过小，甚至都不足 1 像素的千分之几，那就可以不用 call Native 了
        if (!dx.safeEqualToZero() || !dy.safeEqualToZero()) {
    ...
    override fun clipPath(path: Path, clipOp: ClipOp) {
        ...
        val nativePath = path.currentPath as? NativePathImpl ?: return
        viewProxy.clipPath(
    ...
    override fun drawPath(path: Path, paint: Paint) {
        path.pathType = PathType.Native
        nativePaint.sync(paint)
        if (path.currentPath is NativePathImpl) {
            val nativePath = path.currentPath as NativePathImpl
    ...
    override fun drawNullTextImage(paragraphHashCode: Int, width: Int, height: Int) {
        viewProxy.drawNullText(
            cacheKey = paragraphHashCode,
            width = width,
            height = height
        )
    ...
    override fun needRedrawImageWithHashCode(
        ...
        if (experimentalConfig != null && experimentalConfig.textFixLeakType != TMMTextFixLeakType.TMMTextFixLeakTypeNone) {
            return viewProxy.drawImageWithCacheKeyIfNeed(paragraphHashCode, width, height)
        } else {
    ...
    override fun asyncDrawIntoCanvas(
        ...
        viewProxy.asyncDrawIntoCanvas(globalTask, paragraphHashCode, width, height, drawCompleted)
    }
    ...
    private fun drawImageRectImpl(
        ...
        val imageRef = image.asCGImage() ?: return

...
internal class UIViewLayer(
    ...
    private var optimizeForTransform: Boolean = false

    private val canvas = AdaptiveCanvas(experimentalConfig)
    ...
    // 用于 Layer semanticsId
    private var ownerSemanticsId: Int = -1
    ...
    override fun updateLayerProperties(
        ...
        this.optimizeForTransform = scope.optimizeForTransform
    ...
    private fun performDrawLayer(canvas: IOSNativeCanvas, bounds: Rect) {
      // 这里去掉 alpha > 0 的绘制条件判断（无论是否为0绘制应该触发），否则会丢掉 alpha 初始值为 0， 且后不调用 invalidate（） 的节点绘制
        ...
        // 在 Layer 的 canvas 上标记组件绘制
        if (isDebugInspectorInfoEnabled && ownerSemanticsId >= 0) {
            canvas.onBeginComponentDraw(ownerSemanticsId)
        }
        drawBlock(canvas)
        if (isDebugInspectorInfoEnabled && ownerSemanticsId >= 0) {
            canvas.onEndComponentDraw(ownerSemanticsId)
        }
        if (clip) {
            // clipEnd 最好放在 restore 之前
            canvas.clipEnd()
        }
        canvas.restore()
        if (clip) {
    ...
    override fun setOwnerSemanticsId(semanticsId: Int) {
        ownerSemanticsId = semanticsId
    }

    override fun layerProxyId(): Long {
        return viewProxy.getInspectionId()
    }
}

...
import androidx.compose.runtime.tooling.InspectionDataController
...
import androidx.compose.ui.platform.AccessibilityMediator2
...
import androidx.compose.ui.window.Choreographer
...
private class SemanticsOwnerListenerImpl2(
    private val container: UIView,
    private val coroutineContext: CoroutineContext,
    private val getAccessibilitySyncOptions: () -> AccessibilitySyncOptions
) : PlatformContext.SemanticsOwnerListener {
    private var current: Pair<SemanticsOwner, AccessibilityMediator2>? = null

    override fun onSemanticsOwnerAppended(semanticsOwner: SemanticsOwner) {
        if (current == null) {
            current = semanticsOwner to AccessibilityMediator2(
                view = container,
...
private class SemanticsOwnerListenerImpl(
    ...
    override fun onSemanticsOwnerRemoved(semanticsOwner: SemanticsOwner) {
        ...
        if (current.first == semanticsOwner) {
            current.second.dispose()
            // 延迟释放 AccessibilityMediator，以及内部的 Root
...
// region Tencent Code
    private val choreographer: Choreographer? = null,
...
// endregion
    ...
    // region Tencent Code
    override fun render(canvas: Canvas, targetTimestamp: NSTimeInterval, duration: NSTimeInterval, lastFrameTimestamp: NSTimeInterval) {
        val topLeft = getBoundsInPx().topLeft.toOffset()
        canvas.translate(-topLeft.x, -topLeft.y)
        val timestamp = targetTimestamp.toNanoSeconds()
        ChoreographerManager.onVsync(timestamp)
        scene.render(canvas, timestamp)
        choreographer?.onDraw(timestamp, duration.toNanoSeconds(), lastFrameTimestamp.toNanoSeconds())
        canvas.translate(topLeft.x, topLeft.y)
...
// endregion
    ...
    // region Tencent Code
    private val choreographer by lazy { Choreographer() }
    ...
    private val semanticsOwnerListener by lazy {
        ...
        *
        */
        if (ComposeTabService.composeAccessibilityNativeEnable) {
            SemanticsOwnerListenerImpl2(rootView, coroutineContext, getAccessibilitySyncOptions = {
                configuration.accessibilitySyncOptions
            })
        } else {
            SemanticsOwnerListenerImpl(rootView, coroutineContext, getAccessibilitySyncOptions = {
    ...
    init {
        ...
        if (configuration.renderBackend == RenderBackend.UIView) {
            ...
            scene.setLayerFactory(UIViewLayerFactory(renderingView, experimentalConfig, clipChildren))
        }
        // endregion
    ...
    private fun onScrollEvent(
        ...
        val eventType = when(eventKind){
            TouchesEventKind.BEGAN -> PointerEventType.ScrollStart
            TouchesEventKind.MOVED -> PointerEventType.Scroll
            TouchesEventKind.ENDED -> PointerEventType.ScrollEnd
        }
        scene.sendPointerEvent(
            eventType = eventType,
    ...
    private fun ProvideComposeSceneMediatorCompositionLocals(content: @Composable () -> Unit) =
        CompositionLocalProvider(
            ...
            LocalChoreographer provides choreographer,
            content = content
        )
    ...
    // region Tencent Code
    fun updateBoundsInWindow() {
        val newBounds = boundsInWindow?.invoke() ?: calcBoundsInWindow()
        if (scene.boundsInWindow != newBounds) {
            if (scene.boundsInWindow?.size != newBounds.size) {
                renderingComponent.needRedrawSynchronously()
            }
            scene.boundsInWindow = newBounds
        }
    ...
    fun viewDidDisappear(animated: Boolean) {
        ComposeTabService.composeHitTestLog("composeSceneMediator viewDidDisappear")
        scene.processCancelledPointerInputEvent()
        onComposeSceneInvalidate()
    }
    //endregion
    ...
    private var convertParameters: ConvertParameters? = null

    override fun injectConvert(convertParameters: ConvertParameters?) {
        if (!isDebugInspectorInfoEnabled) return
        this.convertParameters = convertParameters
        interactionView.setNodeBlock {
            this.convertParameters?.invoke()
        }
    }
    // endregion

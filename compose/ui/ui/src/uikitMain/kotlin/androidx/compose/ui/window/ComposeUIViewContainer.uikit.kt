...
import androidx.compose.ui.uikit.utils.OVCMPIsMainThread
...
class ComposeUIView(
    ...
    override fun layoutSubviews() {
        if (ComposeTabService.threadProtectionEnable && !OVCMPIsMainThread()) {
            return
        }
    ...
    private fun viewDidDisappear() {
        mediator?.viewDidDisappear(false)
    ...
    override fun willMoveToSuperview(newSuperview: UIView?) {
        ...
        if (configuration.disableAutoTriggerLifecycle) {
            return
        }
    ...
    private fun createRenderingComponent(renderDelegate: RenderingComponent.Delegate) =
        RenderingComponent(
            backend = configuration.renderBackend,
    ...
    private fun initContentIfNot() {
        ...
        mediator.setContentImmediately {
            CompositionLocalProvider(
                LocalLifecycleOwner provides viewLifecycle,
    ...
    // region Tencent Code
    /**
     * 由外部通知 ComposeUIView 其所处的位置发生变化
     */
    fun onContainerOffsetChange(x: Float, y: Float) {
        mediator?.updateBoundsInWindow()
        mediator?.outerContainerOffsetChange(x, y)
    }
    // endregion

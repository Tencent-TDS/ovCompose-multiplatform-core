...
sweepAngleDegrees:(float)sweepAngleDegrees
  forceMoveTo:(BOOL)forceMoveTo {
    ...
    bool isClockwise = sweepAngleDegrees >= 0;
    [self.bezierPath addArcWithCenter:center radius:width / 2 startAngle:startAngle endAngle:endAngle clockwise:isClockwise];
}
...
- (CGRect)getBounds {
    // When the path is empty, return (0, 0, 0, 0) per Compose Path#getBounds contract.
    if (!_bezierPath || _bezierPath.isEmpty) {
        return CGRectZero;
    }
    // Other APIs feed coordinates into UIBezierPath after dividing by density (px -> pt).
    // Here we do the inverse: convert pt back to px so the returned bounds stay in
    // Compose's pixel coordinate space.
    const CGFloat density = TMMComposeCoreDeviceDensity();
    const CGRect bounds = _bezierPath.bounds;
    return CGRectMake(bounds.origin.x * density,
                      bounds.origin.y * density,
                      bounds.size.width * density,
                      bounds.size.height * density);
}

...
static NSString* const gComposeLRUConfig = @"ios_compose_lru_config";
...
+ (instancetype)sharedInstance {
    ...
    dispatch_once(&onceToken, ^{
        NSDictionary *config = [TMMComposeInjectCommonServiceImpl.sharedInstance tmm_getTabMapValueKey:gComposeLRUConfig];
        NSUInteger countLimit = [config[@"countLimit"] unsignedIntegerValue];
        NSUInteger costLimit = [config[@"costLimit"] unsignedIntegerValue];
        memeoryCache = [[TMMComposeMemoryCache alloc] init];
        memeoryCache.name = @"__TMMComposeMemoryCache";
        memeoryCache.countLimit = countLimit > 0 ? countLimit : 30;
        memeoryCache.costLimit = costLimit > 0 ? costLimit: 6 * 1024 * 1025;
    });
    return memeoryCache;
}

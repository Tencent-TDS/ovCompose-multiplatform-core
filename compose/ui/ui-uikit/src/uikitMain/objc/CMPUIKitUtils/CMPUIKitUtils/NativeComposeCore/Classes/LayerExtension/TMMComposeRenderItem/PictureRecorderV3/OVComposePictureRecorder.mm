...
CALayerSaveState PictureRecorder::errorSaveState = CALayerSaveStateCreateSafeGuard();
...
CALayerSaveState &PictureRecorder::safeTopState() {
    if (saveStack.size() > 0) {
        return saveStack[saveStack.size() - 1];
    }
    return PictureRecorder::errorSaveState;
}
...
void PictureRecorder::scale(float sx, float sy) {
    CALayerSaveState &currentState = topState();
    currentState.hasRotateOrScale = true;
    currentState.transformInfo.instantlyTranslate = CGPointMake(currentState.translateX, currentState.translateY);
    currentState.transformInfo.transform = CATransform3DScale(currentState.transformInfo.transform, sx, sy, 1);
}

void PictureRecorder::rotate(float degrees) {
    CALayerSaveState &currentState = topState();
    currentState.hasRotateOrScale = true;
    currentState.transformInfo.transform = CATransform3DRotate(currentState.transformInfo.transform, degrees * (M_PI / 180), 0, 0, 1);
    currentState.transformInfo.instantlyTranslate = CGPointMake(currentState.translateX, currentState.translateY);
}
...
void PictureRecorder::clipEnd() {
    // TODO 这里应该要还原一下 translate 否则会造成 UI 渲染异常
    currentDrawHash = hashMerge(currentDrawHash, DrawingItem::DrawingPopItem.contentsHash);
...
PictureRecorderUpdateInfo PictureRecorder::drawLayer(CALayer *layer) {
    ...
    // 1. drawLayer 的 currentDrawHash 需要有 layer 的 hash 参与运算
    currentDrawHash = hashMerge(currentDrawHash, drawLayerUniqueHash);

    // 2. drawLayer 的 itemHash 和 contentsHash 固定为同一个
...
void PictureRecorder::pushSaveStack(CALayerSaveStateMakeType type) {
    const CALayerSaveState &currentState = safeTopState();
    saveStack.emplace_back((CALayerSaveState) {
        .transformInfo = currentState.transformInfo,
        .translateX = currentState.translateX,
        .translateY = currentState.translateY,
        .makeType = type,
        .hasRotateOrScale = false,
    });
}
...
void PictureRecorder::pushClip() {
    ...
    saveState.transformInfo = CALayerTransform3DInfoIdentity();
...
PictureRecorderUpdateInfo PictureRecorder::draw(TMMNativeDrawingType drawingType, uint64_t drawingContentHash) {
    // 1. 计算一次 recording 过程中的增量 hash，如果结果一样可以直接不用 diff，也不用创建任何 OC 对象
    const CALayerSaveState &saveState = topState();
    ...
    // 2. 计算绘制命令的 hash，注意绘制命令的 hash 只和类型、还有当前类型下的 seqId 有关
    ...
    // 3. 重要⚠️⚠️⚠️：单次 diff 的准则应该是和 item 有关
...
UIView *PictureRecorder::getOrCreateClipView(uint64_t itemHash) {
    ...
    UIView *clipView = nil;
    if (enableCALayerClipOpt) {
        clipView = [[TMMNativeFastClipView alloc] init];
    } else {
        clipView = [[TMMNativeClipView alloc] init];
    }
...
CALayer *PictureRecorder::getOrCreateLayerForDrawing(TMMNativeDrawingType type, uint64_t itemHash) {
    ...
    Class layerClass = TMMDrawingLayerClassFromTypeV2(type);

...
/// 全局自增 ID 计数器
static NSInteger sInteropIdCounter = 1;

/// 全局存储：interopId -> TMMInteropWrapView (弱引用)
static NSMapTable<NSNumber *, TMMInteropWrapView *> *sInteropViewRegistry = nil;
...
@property (nonatomic, assign) NSInteger interopId;
...
+ (void)initialize {
    if (self == [TMMInteropWrapView class]) {
        // 使用弱引用存储，避免内存泄漏
        sInteropViewRegistry = [NSMapTable strongToWeakObjectsMapTable];
    }
}

+ (nullable TMMInteropWrapView *)viewWithInteropId:(NSInteger)interopId {
    @synchronized (sInteropViewRegistry) {
        return [sInteropViewRegistry objectForKey:@(interopId)];
    }
}
...
- (instancetype)init {
    ...
    if (self) {
        ...
        if (TMMLookInDebugEnabel()) {
            // 分配自增 ID 并注册
            @synchronized (sInteropViewRegistry) {
                _interopId = sInteropIdCounter++;
                [sInteropViewRegistry setObject:self forKey:@(_interopId)];
            }
        }
    }
    return self;

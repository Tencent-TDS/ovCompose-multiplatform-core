...
static void OnSurfaceDestroyedCB(OH_NativeXComponent *component, void *window) {
    ...
    // Note: 先调用 aboutToDisappear 再调用 onSurfaceDestroy 在这个 delete render 时就已经访问不到 controller 了
    // TODO: 一个好的方案可能是不信任鸿蒙系统的调用时序，自己维护好 Surface 和 Component 的状态，当前置资源不满足时不会再渲染
    delete render;
}
...
bool XComponentRender::EglMakeCurrentNull() const {
    if (eglDisplay == nullptr) {
        LOGE("XComponentRender: EglMakeCurrentNull: eglDisplay null");
        return false;
    }
...
// 备用调试EGL错误
void XComponentRender::printError() const {
    ...
    switch (error) {
        case EGL_BAD_DISPLAY:
            LOGE("XComponentRender EGL_BAD_DISPLAY: Invalid EGLDisplay\n");
            break;
        case EGL_BAD_CONTEXT:
            LOGE("XComponentRender EGL_BAD_CONTEXT: Invalid EGLContext\n");
            break;
        case EGL_BAD_SURFACE:
            LOGE("XComponentRender EGL_BAD_SURFACE: Invalid EGLSurface\n");
            break;
        case EGL_BAD_MATCH:
            LOGE("XComponentRender EGL_BAD_MATCH: Context and surface are incompatible\n");
            break;
        case EGL_NOT_INITIALIZED:
            LOGE("XComponentRender EGL_NOT_INITIALIZED: EGLDisplay is not initialized\n");
            break;
        case EGL_BAD_ACCESS:
            LOGE("XComponentRender EGL_BAD_ACCESS: Resource is unavailable\n");
            break;
        case EGL_BAD_NATIVE_WINDOW:
            LOGE("XComponentRender EGL_BAD_NATIVE_WINDOW: Invalid native window\n");
            break;
        case EGL_BAD_ALLOC:
            LOGE("XComponentRender EGL_BAD_ALLOC: Memory allocation failed\n");
            break;
        default:
            LOGE("XComponentRender Unknown error: 0x%x\n", error);
            break;
    }
}

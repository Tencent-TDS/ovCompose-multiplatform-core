...
namespace OH {
    ...
    OH_ALWAYS_INLINE void PictureRecorder::resetDrawingItemContentsHash(OHNativeDrawingType type, uint64_t itemHash) {
        assert(type != OHNativeDrawingType::Pop && type != OHNativeDrawingType::DrawLayer);
    ...
    PictureRecorder::SequenceIdInfo PictureRecorder::allocSequenceIdInfo(OHNativeDrawingType type, uint64_t currentContentsHash) {
        const size_t typeIdx = static_cast<size_t>(type);
        SequenceTypeItem &info = sequenceTable[typeIdx];
    ...
    void PictureRecorder::pushSaveStack(RenderNodeSaveStateMakeType type) {
    ...
    void PictureRecorder::pushClip() {
        pushSaveStack(RenderNodeSaveStateMakeType::Clip);
    ...
    void PictureRecorder::popClip() {
        ...
        for (int i = static_cast<int>(saveStack.size()) - 1; i >= 0; i--) {
            ...
            if (saveStateItem.makeType == RenderNodeSaveStateMakeType::Clip) {
                popCount += 1;
            } else {
                break;
            }
    ...
    PictureRecorderUpdateInfo PictureRecorder::draw(OHNativeDrawingType drawingType, uint64_t drawingContentHash) {
        initPropsIfNeeded();
    ...
    void PictureRecorder::rebuildRenderNodeHierarchy(BaseRenderNode& rootRenderNode) {
        ...
        for (size_t i = 0; i < size; i++) {
            ...
            const OHNativeDrawingType drawingType = drawingItem.drawingType;
            switch (drawingType) {
                case OHNativeDrawingType::Save:
                    break;
                case OHNativeDrawingType::Clip: {
    ...
    void PictureRecorder::detachRenderNode(BaseRenderNode& rootRenderNode, OHNativeDrawingType drawingType, uint64_t itemHash) {
        switch (drawingType) {
            case OHNativeDrawingType::Clip: {
                auto it = props->clipPool.find(itemHash);
                if (it != props->clipPool.end()) {
                    auto* clipRenderNode = it->second;
                    props->clipPool.erase(itemHash);
                    rootRenderNode.removeChild(clipRenderNode);
                }
                break;
            }
            case OHNativeDrawingType::DrawLayer: {
                auto iterator = props->renderNodePool.find(itemHash);
                if (iterator != props->renderNodePool.end()) {
                    auto* layerWillBeDelete = iterator->second;
                    props->renderNodePool.erase(iterator);
                    rootRenderNode.removeChild(layerWillBeDelete);
                }
                break;
            }
            default: {
    ...
    void PictureRecorder::diffDrawingItems(BaseRenderNode& rootRenderNode) {
        ...
        if (oldSize > 0 && newSize == 0) {
            for (size_t i = 0; i < oldSize; i++) {
                ...
                const OHNativeDrawingType drawingType = commandToBeDelete.drawingType;
                const uint64_t itemHash = commandToBeDelete.itemHash;

                if (drawingType != OHNativeDrawingType::DrawLayer && drawingType != OHNativeDrawingType::Pop) {
        ...
        bool shouldRebuildRenderNodeHierarchy = false;

        for (size_t i = 0; i < oldSize; i++) {
            bool found = false;
            for (size_t j = 0; j < newSize; j++) {
                if (oldArray[i].itemHash == newArray[j].itemHash) {
                    found = true;
                    break;
                }
            }
            if (!found) {
                ...
                if (commandToBeDelete.drawingType != OHNativeDrawingType::DrawLayer &&
                        commandToBeDelete.drawingType != OHNativeDrawingType::Pop) {
                ...
                shouldRebuildRenderNodeHierarchy = shouldRebuildRenderNodeHierarchy ||
                        (commandToBeDelete.drawingType == OHNativeDrawingType::Clip);
            }
        }

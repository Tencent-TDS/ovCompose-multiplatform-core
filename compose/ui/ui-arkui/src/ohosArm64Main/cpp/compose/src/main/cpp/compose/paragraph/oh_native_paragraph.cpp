...
Paragraph::Paragraph(
    ...
    : text_(std::move(text)),
      textStyleStrategy_(std::move(textStyleStrategy)),
      paragraphStyleStrategy_(std::move(paragraphStyleStrategy)),
      isLayouted_(false),
      fontCollection_(DrawingResourceFactory::createFontCollection()),
      typography_(nullptr, nullptr),
      spanStyles_(spanStyles),
      placeholders_(placeholders),
      fontFamily_(std::move(fontFamily)) {
  LOGI(
      "[Paragraph] Constructor: text='%{public}s', textLength=%{public}zu, "
  ...
  lineMetricsCache_ = std::make_unique<LineMetricsCacheManager>(
      [this]() { return buildLineMetricsCache(); });

  metricsCache_ = std::make_unique<MetricsCacheManager>(
      [this]() { return buildParagraphMetrics(); });
...
void Paragraph::createTypographyResources() {
  ...
  // 创建Typography处理器
  const auto handler = DrawingResourceFactory::createTypographyHandler(
      typoStyle.get(), fontCollection_.get());
  if (!handler.isValid()) {
  ...
  if (spanStyles_.empty() && placeholders_.empty()) {
    ...
    LOGI("[Paragraph] Simple text mode with %{public}zu placeholders",
         placeholders_.size());
    // 只有占位符，没有样式变化
    OH_Drawing_TypographyHandlerPushTextStyle(handler.get(), textStyle.get());
    ...
    for (const auto &placeholder : placeholders_) {
      ...
      if (placeholder.start > currentPos) {
        std::string segment =
            text_.substr(currentPos, placeholder.start - currentPos);
      ...
      placeholderSpan.alignment =
          convertPlaceholderAlignment(placeholder.verticalAlign);
      placeholderSpan.baseline = TEXT_BASELINE_ALPHABETIC;
      placeholderSpan.baselineOffset = 0.0;
      OH_Drawing_TypographyHandlerAddPlaceholder(handler.get(),
                                                 &placeholderSpan);
    ...
    LOGI("[Paragraph] Rich text mode with %{public}zu span styles",
         spanStyles_.size());
  ...
  LOGI("[Paragraph] Typography created successfully: typography=%{public}p",
       typography_.get());
 ...
 */
void Paragraph::applySpanStyles(OH_Drawing_TypographyCreate *handler,
                                OH_Drawing_TextStyle *baseStyle) const {
  if (text_.empty()) {
...
std::vector<Paragraph::StyleOp> Paragraph::convertCutsToOps(
    const std::vector<StyleCut> &cuts) const {
...
void Paragraph::handleStyleAdd(
    const StyleCut &cut, std::vector<const SpanStyleRange *> &activeStyles,
 ...
 * 同时处理占位符插入
 */
void Paragraph::buildParagraphFromOps(OH_Drawing_TypographyCreate *handler,
  ...
  for (size_t i = 0; i < ops.size(); ++i) {
    ...
    // 检查当前段落中是否包含占位符
    while (placeholderIndex < placeholders_.size() &&
           placeholders_[placeholderIndex].start < endPos) {
      const auto &placeholder = placeholders_[placeholderIndex];

      // 添加占位符前的文本
      if (placeholder.start > startPos) {
        OH_Drawing_TypographyHandlerPushTextStyle(handler,
                                                  op.mergedStyle.get());
        std::string segment =
            text_.substr(startPos, placeholder.start - startPos);
      ...
      LOGI(
          "[Paragraph] Inserting placeholder at position %{public}u: "
    ...
    LOGI("[Paragraph] Applied style at pos %{public}u-%{public}u", startPos,
         endPos);
  }
}
 ...
 */
void Paragraph::applySpanStyleToTextStyle(OH_Drawing_TextStyle *textStyle,
  ...
  if (span.textDecoration != TextDecoration::None) {
    OH_Drawing_SetTextStyleDecoration(textStyle,
                                      static_cast<int>(span.textDecoration));
  }
  if (span.shadow) {
...
void Paragraph::layout(const double maxWidth) {
  LOGI(
      "[Paragraph::layout] Starting layout: this=%{public}p, "
      "maxWidth=%{public}.2f",
      this, maxWidth);
...
double Paragraph::getHeight() const {
  if (!typography_.isValid()) return 0.0;
...
double Paragraph::getHorizontalPosition(uint32_t offset,
                                        bool usePrimaryDirection) const {
  // TODO: 实现具体逻辑
  return 0.0;
}

uint32_t Paragraph::getOffsetForPosition(const double dx,
                                         const double dy) const {
  if (!typography_.isValid()) return 0;

  OH_Drawing_PositionAndAffinity *posAndAffinity =
      OH_Drawing_TypographyGetGlyphPositionAtCoordinateWithCluster(
          typography_.get(), dx, dy);

  if (!posAndAffinity) return 0;
...
WordBoundary Paragraph::getWordBoundary(uint32_t offset) const {
  ...
  if (OH_Drawing_Range *range =
          OH_Drawing_TypographyGetWordBoundary(typography_.get(), offset)) {
...
std::vector<TextRect> Paragraph::getRectsForRange(uint32_t start,
                                                  uint32_t end) const {
  ...
  OH_Drawing_TextBox *textBox = OH_Drawing_TypographyGetRectsForRange(
      typography_.get(), start, end, RECT_HEIGHT_STYLE_MAX,
...
std::vector<TextRect> Paragraph::getPlaceholderRects() const {
  ...
  OH_Drawing_TextBox *textBox =
      OH_Drawing_TypographyGetRectsForPlaceholders(typography_.get());

  if (!textBox) {
    LOGE(
        "[Paragraph] getPlaceholderRects: Failed to get placeholder rects from "
        "typography");
  ...
  uint32_t boxCount = OH_Drawing_GetSizeOfTextBox(textBox);
  LOGI(
      "[Paragraph] getPlaceholderRects: Retrieved %{public}u placeholder rects "
  ...
  for (uint32_t i = 0; i < boxCount; ++i) {
    ...
    LOGI(
        "[Paragraph] Placeholder[%{public}u] rect: (%{public}.2f, "
        "%{public}.2f, %{public}.2f, %{public}.2f)",
        i, result.back().left, result.back().top, result.back().right,
        result.back().bottom);
  }
...
void Paragraph::paint(float x, float y) {
  LOGI(
      "[Paragraph::paint] Paint called: this=%{public}p, "
...
void Paragraph::createOrUpdatePositionProperty(float x, float y) {
  if (!posProperty_) {
    LOGI(
        "[Paragraph::createOrUpdatePositionProperty] Creating new position "
        "property: (%{public}.2f, %{public}.2f)",
        x, y);
    posProperty_ = OH_ArkUI_RenderNodeUtils_CreateVector2Property(x, y);
    maybeThrow(OH_ArkUI_RenderNodeUtils_AttachVector2Property(modifier_,
                                                              posProperty_));
  } else {
    LOGI(
        "[Paragraph::createOrUpdatePositionProperty] Updating position "
        "property: (%{public}.2f, %{public}.2f)",
        x, y);
    maybeThrow(
        OH_ArkUI_RenderNodeUtils_SetVector2PropertyValue(posProperty_, x, y));
  }
}
...
void Paragraph::initModifier() {
  if (!modifier_) {
    ...
    maybeThrow(
        OH_ArkUI_RenderNodeUtils_AttachContentModifier(nodeHandle_, modifier_));
    maybeThrow(OH_ArkUI_RenderNodeUtils_SetContentModifierOnDraw(
        modifier_, this, [](ArkUI_DrawContext *context, void *userData) {
          ...
          OH_ArkUI_RenderNodeUtils_GetVector2PropertyValue(data->posProperty_,
                                                           &x, &y);
...
uint32_t Paragraph::getLineForVerticalPosition(const double vertical) const {
  ...
  while (low < high) {
    ...
    if (const LineMetrics &metrics = allMetrics[mid];
        vertical < metrics.bottom) {
...
std::vector<LineMetrics> Paragraph::buildLineMetricsCache() {
  ...
  for (uint32_t i = 0; i < lineCount; ++i) {
    const OH_Drawing_LineMetrics *ohLineMetrics =
        OH_Drawing_TypographyGetLineMetrics(typography_.get());
    LineMetrics metrics;
...
ParagraphMetrics Paragraph::buildParagraphMetrics() const {
  ...
  LOGI(
      "[Paragraph::buildParagraphMetrics] Metrics built: width=%{public}.2f, "
      "height=%{public}.2f, "
      ...
      metrics.width, metrics.height, metrics.minIntrinsicWidth,
      metrics.maxIntrinsicWidth, metrics.lineCount, metrics.alphabeticBaseline,
      metrics.ideographicBaseline, metrics.longestLine,
      metrics.didExceedMaxLines);

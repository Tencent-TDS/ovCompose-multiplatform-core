...
Paragraph::Paragraph(
    ...
    const std::string &fontFamily) : text_(text),
                                     ...
                                     isLayouted_(false) {
    LOGI("[Paragraph] Constructor: text='%{public}s', textLength=%{public}zu, spanStylesCount=%{public}zu, fontFamily='%{public}s'",
...
void Paragraph::createTypographyResources() {
    ...
    // 创建Typography处理器
    auto handler = DrawingResourceFactory::createTypographyHandler(
...
void Paragraph::applySpanStyles(OH_Drawing_TypographyCreate *handler,
    ...
    std::sort(sortedSpans.begin(), sortedSpans.end(),
              [](const SpanStyleRange &a, const SpanStyleRange &b) {
...
WordBoundary Paragraph::getWordBoundary(uint32_t offset) const {
    ...
    OH_Drawing_Range *range = OH_Drawing_TypographyGetWordBoundary(
        typography_.get(), offset);
...
void Paragraph::createOrUpdatePositionProperty(float x, float y) {
    if (!posProperty_) {
        LOGI("[Paragraph::createOrUpdatePositionProperty] Creating new position property: (%{public}.2f, %{public}.2f)", x, y);
        ...
        LOGI("[Paragraph::createOrUpdatePositionProperty] Updating position property: (%{public}.2f, %{public}.2f)", x, y);
...
ParagraphMetrics Paragraph::buildParagraphMetrics() {
    ...
    LOGI("[Paragraph::buildParagraphMetrics] Metrics built: width=%{public}.2f, height=%{public}.2f, "
         ...
         "alphabeticBaseline=%{public}.2f, ideographicBaseline=%{public}.2f, longestLine=%{public}.2f, didExceedMaxLines=%{public}d",
         metrics.width, metrics.height, metrics.minIntrinsicWidth, metrics.maxIntrinsicWidth,
         metrics.lineCount, metrics.alphabeticBaseline, metrics.ideographicBaseline,
         metrics.longestLine, metrics.didExceedMaxLines);

...
LineRenderNode::LineRenderNode() { this->LineRenderNode::initModifier(); }

OH_DrawingNode_Type LineRenderNode::getType() { return OH_DrawingNode_Type::LineNode; };

/**
 * @brief Draws a line with the specified start and end coordinates, width,
 * color, and stroke cap style.
 *
 * This method updates the line's properties, including its start and end
 * points, width, color, and stroke cap.
 *
 ...
 * @param lineColor The color of the line, represented as a 32-bit unsigned
 * integer.
...
void LineRenderNode::createOrUpdateStartPointProperty(const float x, const float y) {
    if (!startPointProperty_) {

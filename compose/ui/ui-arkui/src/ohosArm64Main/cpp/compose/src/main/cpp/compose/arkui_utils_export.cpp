...
Boolean androidx_compose_ui_arkui_utils_xcomponent_makeCurrentNull(void *render) {
    LOGI("androidx_compose_ui_arkui_xcomponent_c_makeCurrentNull render(%{public}p)", render);
    auto xComponentRender = reinterpret_cast<androidx::compose::ui::arkui::utils::XComponentRender *>(render);
    return xComponentRender->EglMakeCurrentNull();
}

...
namespace OH {
    ...
    public:
        ...
        explicit Transform3D(const float* data) noexcept {
            std::memcpy(matrix, data, sizeof(matrix));
        }

        Transform3D(const Transform3D& other) noexcept {
...
#pragma mark - 静态构造方法
        static OH_ALWAYS_INLINE Transform3D Identity() noexcept {
            return Transform3D();
        }
...
#pragma mark - 工具方法
        ...
        OH_ALWAYS_INLINE const float* data() const noexcept {
            return matrix;
        }

        OH_ALWAYS_INLINE float* data() noexcept {
            return matrix;
        }

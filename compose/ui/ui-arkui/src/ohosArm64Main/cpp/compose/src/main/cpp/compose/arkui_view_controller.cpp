...
void ArkUIViewController_setNativeInteropContainer(ArkUIViewController *controller, napi_value back, napi_value fore, napi_value touchable) {
    if (controller == nullptr) {
        LOGE("ArkUIViewController_setNativeInteropContainer: controller is null");
        return;
    }

    if (back == nullptr) {
        LOGE("ArkUIViewController_setNativeInteropContainer: back is null");
        return;
    }

    if (fore == nullptr) {
        LOGE("ArkUIViewController_setNativeInteropContainer: fore is null");
        return;
    }

    if (touchable == nullptr) {
        LOGE("ArkUIViewController_setNativeInteropContainer: touchable is null");
        return;
    }

    androidx_compose_ui_arkui_ArkUIViewController_setNativeInteropContainer(controller, back, fore, touchable);
}
...
void ArkUIViewController_initRenderContext(ArkUIViewController *controller, bool enableCApi, napi_value importFrameMgr,
                                          napi_value rootContent) {
    androidx_compose_ui_arkui_ArkUIViewController_initRenderContext(controller, enableCApi, importFrameMgr, rootContent);
}
...
void ArkUIViewController_onSurfaceCreated(ArkUIViewController *controller, OH_NativeXComponent *component, uint64_t width, uint64_t height) {
    if (controller == nullptr) {
...
void ArkUIViewController_onIdle(ArkUIViewController *controller, uint64_t timeLeft) {
    if (controller == nullptr) {
        LOGE("ArkUIViewController_onIdle: controller is null");
        return;
    }
    long long longTimeLeft = static_cast<long long>(timeLeft);
    androidx_compose_ui_arkui_ArkUIViewController_onIdle(controller, longTimeLeft);
}

void ArkUIViewController_draw(ArkUIViewController *controller, napi_value canvas) {
    if (controller == nullptr) {
        LOGE("ArkUIViewController_onIdle: controller is null");
        return;
    }
    androidx_compose_ui_arkui_ArkUIViewController_draw(controller, canvas);
}

void* ArkUIViewController_getJsNode(ArkUIViewController *controller) {
    if (controller == nullptr) {
        LOGE("ArkUIViewController_onIdle: controller is null");
        return nullptr;
    }
    return reinterpret_cast<napi_value>(androidx_compose_ui_arkui_ArkUIViewController_getJsNode(controller));
}
...
void ArkUIViewController_dispatchTouchEvent(ArkUIViewController *controller, void *nativeTouchEvent,
    ...
    if (controller == nullptr || nativeTouchEvent == nullptr) {
...
void ArkUIViewController_onAxisEvent(ArkUIViewController *controller, void *event) {
    if (controller == nullptr || event == nullptr) {
        LOGE("ArkUIViewController_onAxisEvent: controller is null");
        return;
    }
    androidx_compose_ui_arkui_ArkUIViewController_onAxisEvent(controller, event);
}
    
void ArkUIViewController_onMouseEvent(ArkUIViewController *controller, void *event) {
    if (controller == nullptr || event == nullptr) {
        LOGE("ArkUIViewController_onMouseEvent: controller is null");
        return;
    }
    androidx_compose_ui_arkui_ArkUIViewController_onMouseEvent(controller, event);
}
    
void ArkUIViewController_onPinchEvent(ArkUIViewController *controller, void *event) {
    if (controller == nullptr || event == nullptr) {
        LOGE("ArkUIViewController_onPinchEvent: controller is null");
        return;
    }
    androidx_compose_ui_arkui_ArkUIViewController_onPinchEvent(controller, event);
}
    
bool ArkUIViewController_onKeyEvent(ArkUIViewController *controller, void *event) {
    if (controller == nullptr || event == nullptr) {
        LOGE("ArkUIViewController_onKeyEvent: controller is null");
        return false;
    }
    return androidx_compose_ui_arkui_ArkUIViewController_onKeyEvent(controller, event);
}
...
void ArkUIViewController_updateDensity(ArkUIViewController *controller, double density) {
    if (controller == nullptr) {
        LOGE("ArkUIViewController_updateDensity: controller is null");
        return;
    }
    androidx_compose_ui_arkui_ArkUIViewController_updateDensity(controller, static_cast<float>(density));
}
...
void ArkUIViewController_setExtraParam(ArkUIViewController *controller, const std::string &key,
                                           const std::string &value) {
    if (controller == nullptr) {
        LOGE("ArkUIViewController_setExtraParam: controller is null");
        return;
    }
    const char *keyStr = key.c_str();
    const char *valueStr = value.c_str();
    androidx_compose_ui_arkui_ArkUIViewController_setExtraParam(controller, keyStr, valueStr);
}
